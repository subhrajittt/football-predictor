...
def calculate_elo(data, k=20, initial_rating=1500, home_advantage=100):
    ...
    ratings = {}  # (league, team) -> rating
    home_elo_list, away_elo_list = [], []

    for row in data.itertuples():
        league = row.League
        home_key = (league, row.HomeTeam)
        away_key = (league, row.AwayTeam)
        ...
        ratings[home_key] = home_rating + k * (actual_home - exp_home)
        ratings[away_key] = away_rating + k * (actual_away - exp_away)
