...
For every match, the pipeline builds three sets of features, all calculated so that a match never has access to information from its own result or from matches after it (no data leakage):
...
These feed into a logistic regression model, one per league, which outputs win/draw/loss probabilities for any matchup.
...
The naive baseline here is "always predict the most common outcome" (usually home win) — a model that can't beat that isn't learning anything real. Every league's model does, by 5-14 points depending on the league.

**Draws are hard to predict**, consistently across all five leagues — precision and recall on draws are weak, sometimes close to zero. This is a well-documented limitation in football prediction generally, not a bug: draws don't carry as strong a statistical signal as clear favourite/underdog situations. I tried forcing the model to pay more attention to draws using `class_weight='balanced'`, which did produce more draw predictions but dropped overall accuracy noticeably (Premier League: 50.7% → 46.7%). I kept the standard model as the real one, since the trade-off wasn't worth it, but the balanced version is still in the code for comparison.

## Probability Calibration

While testing the app, I noticed something worth checking properly: predictions for clearly strong teams (e.g. Arsenal vs Chelsea) looked overconfident — 70%+ win probabilities for matchups between two genuinely competitive clubs, which didn't match how bookmakers or pundits would actually price it.

I built a calibration check to test this properly rather than go on a gut feeling: bucket every prediction by confidence level, then check whether the model's stated confidence actually matched its real accuracy in that bucket. The result confirmed the instinct — in Premier League specifically, the model's most confident bucket (80-100%) was right only 73% of the time, a 12-point gap. Other leagues showed smaller but similar patterns.

The likely cause: Elo, recent form, and head-to-head are correlated features that often point the same direction, and logistic regression doesn't know that — it treats each one as independent evidence, which compounds confidence more than it should.

Fix: wrapped the model in scikit-learn's `CalibratedClassifierCV`, which rescales output probabilities to better match real-world accuracy. Re-running the same calibration check afterward, the overconfidence mostly disappeared — the Premier League's top bucket, for example, went from a 12-point gap to under 2 points. Arsenal vs Chelsea's predicted probability dropped from 74.3% to 66.8%, which reads as far more realistic.
...
Result: 380 out of 380 matches matched exactly, with zero result discrepancies (after fixing one team-naming inconsistency — the API uses "Sheffield Utd," the CSV source uses "Sheffield United").
...
**Squad market value**, as a way to account for transfers — the idea being that a team that sold its best player and bought a replacement looks identical to Elo/form until enough matches happen to reflect it. I looked at pulling Transfermarkt squad values as a proxy, but decided against it: transfer fees don't reliably predict on-pitch performance (there are plenty of expensive signings that flopped and cheap ones that overperformed), and a proper version of this feature would need squad values calculated per season rather than one current snapshot applied across six years of historical data. Elo and form are derived directly from match outcomes, which is a more honest signal than what a club spent in the transfer market.
...
- **Champions League isn't included.** This was part of the original plan, but it's a genuinely different, harder problem than adding another domestic league — Elo/form built from domestic matches alone doesn't capture cross-league dynamics well, and comparing team strength across ~30+ different leagues and competition levels isn't something six seasons of domestic data can really support. Planned as a future phase, not attempted here.
- **Domestic cups** (FA Cup, Copa del Rey, etc.) aren't included yet, mainly for scope reasons — a reasonable next addition, since the data source and pipeline already support it structurally.
...
├── src/
│ ├── data_prep.py # loads and combines raw seasons, tags league + season
...
│ ├── update_data.py # rebuilds dataset + retrains from raw/
│ ├── api_client.py # API-Football integration
│ └── compare_sources.py # validates API data against CSV data
├── app.py # Streamlit app, league + team selectors, live predictions
└── requirements.txt
...
- Domestic cup competitions (FA Cup, Copa del Rey, DFB-Pokal, Coupe de France)
